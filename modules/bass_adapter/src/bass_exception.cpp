...
std::string error_code_to_str() {
    switch (BASS_ErrorGetCode()) {
        ...
        default:
            return "error code isn't even unknown, hmmmm....";
    }
}
}  // namespace

namespace audio {
BassException::BassException(std::string error_string)
    : err_str_{std::move(error_string) + ", error code is: " + error_code_to_str()} {}

const char* BassException::what() const noexcept { return err_str_.c_str(); }
}  // namespace audio
